  ...
  const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File }) => {
    ...
    const uploadWithTimeout = async (files: File[], ms = 120000) => {
      try {
        ...
        // retry once without a timeout in case of transient slowness
        try {
          return await fs.upload(files);
        } catch (err2) {
          throw err2;
        }
      }
    ...
    try {
      ...
      const uploadedImage: any = await uploadWithTimeout([imageFile.file]);
      if (!uploadedImage || !uploadedImage.path) throw new Error('Failed to upload image');
